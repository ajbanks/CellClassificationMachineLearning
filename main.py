...
if __name__ == '__main__':
	...
	if downSampleFlag:
		...
		if crossValidateFlag:
			...
			while iterator < 10:
				...
				if classifier == 0:
					...
					guassianNB_predictionResults = gnb(trainingFolds, testingData, trainingKeys, testingDataKey, crossValidateFlag)
			...
			if classifier == 0:
				# ***************** GUASSIAN NB *****************
				gnb(data.getDSTrainingData(), data.getDSTestingData(), data.getDSTargetValues(),
					data.getDSTestingDataTargetValues(), crossValidateFlag)
				# ***************** END GUASSIAN NB *****************

			elif classifier == 1:
				# ***************** RBF SVC *****************
				rbfSVC(data.getDSTrainingData(), data.getDSTestingData(), data.getDSTargetValues(),
					data.getDSTestingDataTargetValues(), crossValidateFlag)
				# ***************** END RBF SVC *****************
		...
		if crossValidateFlag:
			...
			if classifier == 0:
				# ***************** GUASSIAN NB *****************
				gnb(data.getTrainingData(), data.getTestingData(), data.getTrainingDataTargetValues(),
					data.getTestingDataTargetValues(), crossValidateFlag)
				# ***************** END GUASSIAN NB *****************

			elif classifier == 1:
				# ***************** RBF SVC *****************
				rbfSVC(data.getTrainingData(), data.getTestingData(), data.getTrainingDataTargetValues(),
					data.getTestingDataTargetValues(), crossValidateFlag)
				# ***************** END RBF SVC *****************
